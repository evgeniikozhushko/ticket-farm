...
1. Enable Cloud Backup and PITR
  In Atlas:
2. Open the production cluster.
3. Go to Backup.
4. Enable Cloud Backup.
5. If available for your tier, enable Continuous Cloud Backup / Point-in-Time Restore.
6. Use at least a 7-day PITR window for early production.
7. Set snapshot retention, for example:
...
1. Rotate MongoDB Credentials Before Launch
...
1. Test a Real Production Signup
  After deploying with production env vars:
2. Create or confirm a production organization exists.
3. Open the public org URL:

  [https://ticketfarm.ca/](https://ticketfarm.ca/)[

1. Submit a test signup.
2. Verify in Atlas that a document was created in registrants:
...
1. Verify today’s lotteries row exists and has:
...
1. Submit the same email again and confirm the app shows the duplicate-entry message, not
the generic error.
